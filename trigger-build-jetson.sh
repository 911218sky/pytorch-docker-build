...
VERSION FORMAT (same as x86):
    TORCH-cudaCUDA-pyPYTHON
    Example: 2.9.0-cuda12.8-py3.11

EXAMPLES:
    # Build single version
    ./trigger-build-jetson.sh 2.9.0-cuda12.8-py3.11
...
# Parse version string like "2.9.0-cuda12.8-py3.11" into JSON for Jetson
parse_version() {
    local ver="$1"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -f|--file)
            ...
            ;;
        *)
            # Parse version strings (same format as x86)
            if [[ -z "$VERSIONS" ]]; then
...
echo "$VERSIONS" | jq -r '.[] | "  - PyTorch \(.torch)-cuda\(.cuda)-py\(.python)"'
echo
